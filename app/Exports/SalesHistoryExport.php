...
class SalesHistoryExport implements FromArray,ShouldAutoSize,WithHeadings
{
    ...
    /**
    * @return \Illuminat    e\    Support\Collection
    */
    ...
    public function array() :array
    {
        ...
        if($this->customer_id == 0 && $this->sales == 0){
            ...
            $vouchers = Voucher::whereBetween('voucher_date',[$this->from_date, $this->to_date])->where('sales            _customer_id',$this->customer_id)->with('counting_unit')->get();
             $item_lists = array();
            foreach($vouchers as $voucher){
                foreach($voucher->counting_unit as $counting_unit){
                    $voucher_code = $voucher->voucher_code;
